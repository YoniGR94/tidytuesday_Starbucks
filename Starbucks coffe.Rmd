...
rf_grid <- grid_regular(mtry(range(15, 40)), min_n(range(10,30)), levels = c(4, 3))
...
##try to bake at last
#____#
test_baked<- dolche_de_leche(shoes_train_YG) %>% prep(shoes_train_YG) %>% bake(shoes_test_YG) 
...
YG_model_2_tuned_fit<-
  YG_model_2_tuned %>%  fit_xy(x = bake_em_coffe$template[,-30],
...
dolche_de_leche_hash<- function(df)
  {
  recipe(data=  df,calories~.) %>% 
  update_role(id, new_role = "id") %>% ## ADD THIS
  textrecipes::step_tokenize(seller_notes) %>%                                        #added hash
  textrecipes::step_texthash(seller_notes, signed = TRUE, num_terms = 12) %>%       #added hash
...
rf_full_hash<-      dolche_de_leche_hash(shoes_train_YG) %>% prep(shoes_train_YG) %>% bake(new_data= shoes_train_YG)
rf_full_test_hash<- dolche_de_leche_hash(shoes_train_YG) %>% prep(shoes_train_YG) %>% bake(new_data= shoes_test_YG)
```
...
mod_glmnet_final <- YG_model_1_hasher %>%
  fit(calories ~ ., data = rf_full_hash)# %>%

mod_glmnet_final %>% 
  predict(new_data = rf_full_test_hash)
...
true_model_rf_hash_set<- true_model_rf_hash %>% 
  fit_xy(x = rf_full_hash[,-(which(colnames(rf_full_hash)== 'calories'))],
         y= rf_full_hash$calories)
...
true_model_rf_hash_to_csv<-  
  predict(true_model_rf_hash_set ,new_data = rf_full_test_hash) #%>%
