...
cbind(star_train_te$calories, rep(mean_calories, nrow(star_train_te))) %>%
  ...
  geom_violin()+geom_hline(yintercept= mean_calories, size= 1.1, col= "Red")
```
...
dolche_de_leche<- function(df)
  ...
  step_normalize(all_numeric( ), -has_role('id'),-has_role("discard"),-has_role("discard"),-all_outcomes()) %>%
  step_nzv (all_numeric() , -has_role('id'),-has_role("discard") ,-all_outcomes(),freq_cut = 99/1) %>% 
...
mod_glmnet_final %>% 
  predict(new_data = data_full_test) %>%   cbind(cafe_test$product_name) %>%
   rename(calories.pred= 1)# %>% write_csv("true_model_glmnet_3fixe_pred_Yoni.csv")
...
YG_model_2_tuned_fit<-
  YG_model_2_tuned %>%  fit_xy(x = bake_em_coffe$template[,-c(1,25)],
                             y= bake_em_coffe$template$calories) 
YG_model_2_tuned_pred<- YG_model_2_tuned_fit %>% predict(new_data = test_baked$template)

#YG_model_2_tuned_pred<- YG_model_2_tuned_fit %>% predict(new_data = data_full_test)
